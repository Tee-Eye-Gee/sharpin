...
see above), backlog #2 (Personal Analytics UI) per the original sequencing
call logged in the AccountSync spec (§7), or the Logout/Account-reorg work
already investigated in `docs/specs/logout-and-account-reorg-investigation.md`
— subject to the forward-dependency note above if Logout is picked up next.
(That doc's own backlog numbering for Logout/Account-reorg wasn't reconciled
against the AccountSync spec's #2/#7 numbering as part of this build — don't
assume they refer to the same slot without checking.)
